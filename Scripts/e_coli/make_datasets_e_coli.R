...
savefile <- "./jan_data/e_coli_all_feats.rds"
...
peptides.list <- epitopes %>%
  ...
  make_data_splits(proteins    = proteins,
                   ...
                   #return_front = 21,
                   ncpus = ncpus) %>%
  calc_features(local.features = c("AAC", "DC", "TC", "CTDC", "CTDT", "CTDD",    # <--- local features are calculated for the 15-AA windows and added to peptides.list$df
  "CTriad", "SOCN", "QSO", "ScalesGap", "Entropy", "Atoms", "MolWeight", "AAtypes", "BLOSUM"),
                global.features = c("AAC", "DC", "TC", "CTDC", "CTDT", "CTDD",    # <--- global features are calculated at the full protein level and added to peptides.list$proteins (needs a data join on df later, before training the models)
                                    "CTriad", "SOCN", "QSO", "ScalesGap", "Entropy",
                                    "Atoms", "MolWeight", "AAtypes", "BLOSUM"),
                ncpus = ncpus) %>%
  save_peptide_list("./jan_data/e_coli_all_feats")
