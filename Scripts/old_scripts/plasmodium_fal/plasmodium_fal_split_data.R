...
# Same as above 
# orgID <- 418107 # Plasmodium (Laverania) - Subgenus
# plasmodium_laverania <- readRDS("./jan_data/plasmodium_fal_data/plasmodium_laverania_data.rds")
# plasmodium_laverania <- plasmodium_laverania$df

# orgID <- 5820 # plasmodium - Genus
plasmodium <- readRDS("./jan_data/plasmodium_fal_data/plasmodium_data.rds")
plasmodium <- plasmodium$df
plasmodium <- plasmodium[plasmodium$Info_PepID %!in% unique_id_test_plas_fal, ]
write.csv(plasmodium, "plasmodium.csv")

# Same as above
# orgID <- 1639119 # Plasmodiidae - Family 
# plasmodiidae <- readRDS("./jan_data/plasmodium_fal_data/plasmodiidae_data.rds")
# plasmodiidae <- plasmodiidae$df

# Same as above
# orgID <- 5819 # Haemosporida - Order
# haemosporida <- readRDS("./jan_data/plasmodium_fal_data/haemosporida_data.rds")
# haemosporida <- haemosporida$df

# orgID <- 422676 # Aconoidasida - Class
aconoidasida <- readRDS("./jan_data/plasmodium_fal_data/aconoidasida_data.rds")
aconoidasida <- aconoidasida$df
aconoidasida <- aconoidasida[aconoidasida$Info_PepID %!in% unique_id_test_plas_fal, ]
write.csv(aconoidasida, "aconoidasida.csv")

# orgID <- 5794 # Apicomplexa - Phylum
apicomplexa <- readRDS("./jan_data/plasmodium_fal_data/apicomplexa_data.rds")
apicomplexa <- apicomplexa$df
apicomplexa <- apicomplexa[apicomplexa$Info_PepID %!in% unique_id_test_plas_fal, ]
write.csv(apicomplexa, "apicomplexa.csv")

# Same as above
# orgID <- 33630 # Alveolata - Clade
# alveolata <- readRDS("./jan_data/plasmodium_fal_data/alveolata_data.rds")
# alveolata <- alveolata$df
